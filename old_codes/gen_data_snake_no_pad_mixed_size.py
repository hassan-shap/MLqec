...
import numpy as np
import random
from code_utils import code_initialization, decoder, snake_coord
    ...
    with open(JSON_PATH, 'w') as json_file:
        for _ in range(num_batch):
            for i_d, d in enumerate(d_list):
                ...
                perm_mat = snake_coord(d, stab_matrices)
                # shift by one
                ...
                for _ in range(Niter//num_batch):
...
for p_err in p_err_list:
    fname = f"datasets/train_eos_snake_no_pad_mixed_{d_max}_p_{p_err:.2f}.json"
    # fname = f"datasets/val_eos_snake_no_pad_mixed_{d_max}_p_{p_err:.2f}.json"
