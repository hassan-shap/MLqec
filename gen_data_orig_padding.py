...
def data_file_gen(d,JSON_PATH):
    ...
    with open(JSON_PATH, 'w') as json_file:
        ...
        for iter in range(Niter):
            ...
            syndrome_train = np.zeros(d**2+1 )
            syndrome_train[1:d**2] = syndrome
            syndrome_train[d**2:] = 3
            syndrome_train[0] = 3
...
# d_max = 7
d_list = [11] # code distance, must be an odd number 
Niter = 10000 # number of random iterations for error
p_err_list = np.arange(0.01,0.31,0.01) # 
for p_err in p_err_list:
    for d in d_list:
        # fname = f"datasets/train_eos_original_d_{d}_p_{p_err:.2f}.json"
        # fname = f"datasets/val_eos_original_d_{d}_p_{p_err:.2f}.json"
        fname = f"datasets/test_eos_original_d_{d}_p_{p_err:.2f}.json"
